...
（参考我写的另外一篇文章 《两万字科普：AI 为什么会编程——原理、历史与未来》）
...
## 4.3 模型成为零件：Agent 和系统级创新

到这里，一个悄悄的转换发生了。

过去十年，模型工程的目标几乎都集中在一件事：让单个模型变得更强。再大、再快、再准、再听话，所有努力指向"这个模型"本身。

到2024年下半年，重心开始迁移。工程的目标变成"在模型之外搭系统"。模型从最终产品，变成了系统里的一个零件。

这个转换说出来很轻，量级却不小。它意味着接下来AI的进步重心，从"模型再涨多少分"，转到"用现有模型能搭出多复杂的东西"。Agent就是这个转向的典型代表。

先说传统SaaS是什么。SaaS就是Software as a Service，软件不装在你本地，而是跑在云上，你按账号、席位、订阅费来用。CRM、财务系统、HR系统、项目管理工具、工单系统、数据看板，都是典型SaaS。传统SaaS的核心交互是表单、按钮、菜单、流程。人负责理解业务目标，再把目标拆成一个个操作：点哪里、填什么字段、查哪张表、发哪封邮件。

AI-powered SaaS是第一步升级。它通常是在原来的SaaS里加一层AI能力：自动总结会议，生成销售邮件，帮客服改写回复，给报表做自然语言解释，或者根据知识库回答问题。底层流程大体还在，用户还是在原来的系统里点按钮，只是其中某些步骤由LLM帮你写、帮你总结、帮你搜索。这个阶段，AI更像一个嵌在SaaS里的聪明助手。

Agent再往前走一步。它不是只帮你写一段话，而是开始接管"从目标到步骤"这段工作。你不再说"打开客户页、筛选北京客户、写邮件、更新字段"，而是说"帮我把这批沉默客户重新激活"。Agent要自己拆任务：先查客户历史，再分组，再生成邮件，再调用CRM发出，再记录跟进状态，再把异常情况交还给人。传统SaaS里，人是流程的驾驶员。Agent模式里，人更像设定目标和边界的人，模型负责在软件系统里开车。

这个想法不是2024年才出现的。ChatGPT刚火的时候，大家已经在想：如果模型能听懂人的话，能不能让它不只是聊天，而是接上软件、数据库、搜索、文件、支付、邮件、日历，替人完成一个任务？2023年3月，OpenAI推出ChatGPT plugins，让ChatGPT可以调用浏览器、代码解释器和第三方服务。几个月后，function calling进入API，开发者可以把一个外部函数描述给模型，让模型判断什么时候调用它。

与此同时，开源社区也在用传统软件工程的方式接入LLM。LangChain是最典型的代表。它2022年底出现，2023年迅速流行，核心价值不是让模型本身更聪明，而是把Prompt、模型调用、向量数据库、工具、外部API、记忆、链式流程包装成开发者熟悉的组件。过去写一个SaaS系统，是前端、后端、数据库、API、队列、权限、日志。LangChain这类框架做的事情，是把LLM也变成这套软件工程里的一个可编排模块。

这就是为什么Agent重要。它不是一个更会聊天的机器人，而是LLM进入软件系统之后形成的新应用形态。给它一个任务："帮我研究三家公司，下周给一份对比报告"，它自己去查公开资料、调用搜索API、调用计算工具、写代码、整理结果、最后写出一篇带数据带引用的报告。模型负责理解、规划和生成，传统软件负责权限、数据、工具、执行和记录。

2023年的AutoGPT把这个想象推到公众面前。它让模型自己分解任务、自己给自己下指令、自己调用工具，再循环执行。效果其实很不稳定，容易跑偏、卡住、烧token，但它证明了一个方向：LLM可以被放进一个循环里，变成能计划、能行动、能重试的软件代理。

注意一个细节：这些变化里，没有任何单个模型突然变强。变的是围绕模型搭出来的脚手架：工具调用协议，任务分解流程，长程记忆，多步重试，子任务编排，日志和评测。RAG也是同一种类型。它让模型接外部知识库，不必重训就能更新事实。多模型协作也是同一种类型，大模型负责规划，小模型负责执行。长期记忆也是同一种类型，让Claude、ChatGPT可以跨对话保留上下文。

所以，Agent基本可以看作SaaS的一次深层升级，但它不只是"给SaaS加一个聊天框"。聊天框只是入口，真正的变化在后面：模型要能读懂用户目标，拆出步骤，选择工具，调用API，读取结果，发现错误，再决定下一步。CRM里，它可以自动整理客户背景、生成跟进邮件、更新字段。财务系统里，它可以查发票、对账、提醒异常。研发系统里，它可以读issue、改代码、跑测试、发PR。企业软件从"表单和按钮"往"目标和流程"迁移。

这件事还在早期。今天的Agent仍然容易幻觉、权限边界难做、长任务会跑偏、出错责任也不清楚。但方向已经很明确：当模型强到一定程度，工程的重心就会从"优化模型"转到"用模型搭东西"。这是过去70年软件工业一直在做的事，AI终于走到这一步。
