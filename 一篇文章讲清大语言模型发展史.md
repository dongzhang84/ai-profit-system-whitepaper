...
平时我们聊天、API调用、让模型写代码，这叫推理或服务。它也跑在GPU、TPU或专门的推理加速芯片上，只是目标从"训练一个新模型"变成"把已经训练好的模型跑得快、便宜、稳定"。普通聊天消耗的是一次生成的token。推理模式更贵，因为模型在最终回答前还会生成大量内部思考token，占用芯片更久。你看到的是一句答案，机器背后可能已经跑过一大段推演。
