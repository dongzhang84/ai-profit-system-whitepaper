...
自主学习不是翻开一本 Python 教程从第一页看到最后一页，也不是把一份 API 文档从头啃到尾。那是书本时代的打法，2026 年已经过时了。今天的做法是带着问题去找 AI 对话。打开 Claude 或 ChatGPT，把一个问题抛给它，让它解释。但别停在第一个答案，追着问"为什么"、"和 X 什么区别"、"有没有例外情况"，直到你能用费曼法把答案讲给朋友听。AI 是你的"问题提示机"，要在你每一步触发更深的追问。

下面这 20 个问题是这一层会逼你搞清楚的。一个一个问下去，每个问题都往深追三四层。别让 AI 一上来就给你一份"一页纸总结"，那样你学不到东西：

1. API 到底是什么？为什么 LLM 服务商要以 API 形式暴露，而不让你下载模型在本地跑？
2. 一次 API 调用背后完整发生了什么？从你按 Enter 到拿到回复，中间经过哪些机器、协议、数据格式？
3. JSON 在 API 通信里扮演什么角色？不用 JSON 可以用什么？
4. 什么是 REST API？LLM 的 API 是 REST 吗？
5. HTTP 和 WebSocket 在 LLM 场景里分别用于什么情况？
6. 为什么 Python 成了 AI 的主流语言？这段历史是怎么发生的？
7. Python 的 asyncio 是做什么的？为什么 LLM 调用常用异步？
8. virtualenv、venv、conda、poetry 这些工具各自解决什么问题？你应该用哪个？
9. system prompt、user prompt、assistant 这三个角色为什么要这么分？
10. 什么是 context window？它为什么有上限？超了会发生什么？
11. token 是什么？为什么按 token 计费，而不按字符或请求？
12. temperature、top_p、top_k 各自控制什么？改了输出会怎么变？
13. streaming 返回有什么好处？底层协议和一次性返回有什么区别？
14. API key 为什么不能提交到 git？如果泄露了会发生什么？
15. rate limit 是什么？遇到 HTTP 429 应该怎么处理？
16. 什么是 exponential backoff？为什么重试要指数退避？
17. 怎么让 LLM 每次输出都是合法的 JSON？json_mode 和 structured output 是怎么实现的？
18. few-shot 示例放在 prompt 的哪个位置效果最好？为什么？
19. chain-of-thought 的触发真的只靠 prompt 里加一句话吗？它为什么会有效？
20. 怎么量化评估一个 LLM app 的质量？业界有哪些指标和框架？

把这 20 个问题聊明白之后，配合三个项目把它沉淀下去。

**项目 1 · 命令行 chatbot。** 调用一个 LLM API（OpenAI、Claude、DeepSeek 任选），写一个最简单的命令行聊天工具。做完用费曼法自测：关掉编辑器，把这个程序每一步干什么讲给朋友听。

**项目 2 · Web 界面 + prompt engineering。** 把命令行 chatbot 套上 Web 界面（Gradio、Streamlit 或 Next.js 都行），在项目里踩 prompt engineering 的具体坑。

**项目 3 · 部署上线。** 把 app 部署到公网可访问的地方（Vercel、Modal、Railway 这类 serverless 平台），让真的用户能用。

做完三个项目 + 上面 20 个问题你都能自如讲出来，这一层就通关了。通关标准：把部署版本讲给另一个做技术的朋友听，他能听明白从"用户按 Enter"到"GPT 返回答案"中间发生了什么。

## 第二层：能搭一个 RAG 系统

这一层的目标是让 LLM 能接入外部知识，不再只能依赖训练时内置的那些数据。

**项目 4** · 单文档 RAG。选一本书或一份 PDF 报告，用 embedding 模型（OpenAI embeddings 或 bge-small）加向量数据库（ChromaDB、Pinecone 任选），搭一个能对这份文档问答的简单 RAG。

**项目 5** · 复杂 RAG。做 chunking 策略对比（按段落、按 token、按语义），加上多路召回（dense 加 BM25 keyword）和重排模型（cohere rerank 或 bge-reranker），用 hit rate、MRR 这些指标评估检索质量。

**项目 6** · 垂直领域 RAG 产品。选一个你熟悉的 domain，比如你自己的笔记、公司内部文档、某个行业知识库，把 RAG 做到能让目标用户真的用起来，接住真实反馈。

做完这层，你对 RAG 的理解从"能跑"升级到"会调"，也会第一次正面碰到"用户说检索不准"这类真实产品问题。
