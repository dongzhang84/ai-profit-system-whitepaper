...
CV 的故事要从 1980 年代末讲起。Yann LeCun（中文圈常译作"杨立昆"，也有人写成"杨乐坤"）在贝尔实验室做手写数字识别，用的是反向传播加卷积网络。美国邮政当年就用这套系统自动分拣信封上的邮编。这是深度学习第一次在生产环境里真刀真枪干活，LeCun 本人也因为这一支工作拿了 2018 年的图灵奖。

下一个大转折点在 2009 年。斯坦福的李飞飞（Fei-Fei Li）牵头做出 ImageNet，一个有 1400 万张带人工标注的图像数据集。她的贡献不是某个具体算法，而是给整个深度学习提供了"足够的燃料"。2012 年 AlexNet 在 ImageNet 图像分类比赛上碾压传统方法，深度学习这一波工业革命从这里真正起步。

后来 CV 还出过几次"神级应用"。2016 年 DeepMind 的 AlphaGo 在围棋上击败李世石，它的策略网络和价值网络本质上就是把 19×19 的棋盘当图像来处理的卷积神经网络（虽然整个系统的亮点是加上了强化学习和蒙特卡洛树搜索）。医疗影像识别在几类癌症筛查和眼底病变检测上达到甚至超过专业放射科医生的水平。自动驾驶这一整个产业靠的是多路摄像头加 CV 模型做实时目标检测、车道线识别、障碍物判断。过去十年特斯拉、Waymo、Cruise 几乎全是 CV 工程师的主场。

到了 2026 年，CV 的焦点已经从"识别"转向"生成"。Stable Diffusion、Midjourney、DALL-E、Flux 这一批工具底层当然还是 CV 的延伸，仍然需要理解图像的像素分布、结构、风格，只是目标从"判断这是什么"变成了"画出一张符合描述的图"。技术栈的重心也从过去的 CNN，转到了 Vision Transformer（ViT）和 diffusion models 这两条主线。

所以今天学 CV 要想清楚：经典的分类、检测、分割那条线已经不是主流就业方向，除非你锁定医疗影像、自动驾驶、工业质检、机器人视觉这几个仍然有硬需求的垂直行业。更火的是 CV 和 LLM 结合的多模态（CLIP、GPT-4V、Sora），以及图像生成方向。但后者作为纯"工程岗"的饭碗主要集中在几家做 image generation 产品的公司，岗位池比 LLM 本身窄得多。

## 1.3 NLP / LLM（大语言模型）

NLP 原本是让机器处理语言的整个学科，涵盖翻译、情感分析、实体识别、摘要、问答这一大摞任务。但从 GPT-3 开始，原来散落的这些任务几乎都被一个通用的大语言模型统一接住了。所以今天学 NLP 基本等同于学 LLM，包括预训练的原理、各种微调方法、怎么写 prompt、怎么搭 RAG、怎么做 function calling 让模型调工具。

LLM 背后真正需要吃透的数学只有一个：Transformer 的 attention 机制。再加上一点强化学习的基本工具（PPO、DPO）和优化器知识。这是 2026 年最火也是最容易找到工作的方向，岗位需求、薪资水平、创业机会三条线都压着其他方向。如果你是零基础转 AI 且目标是找工作，直接从这里切入，别的方向真需要时再补。
...
多模态（CLIP、GPT-4V、Sora 这一支）算 CV 和 LLM 的结合，2026 年最热的新兴领域，等你把 LLM 主线走完自然会接触到。语音识别和合成（ASR/TTS）相对小众，岗位少，除非你对语音本身特别感兴趣，跳过。强化学习在 LLM 里以 RLHF 和 DPO 的形式存活下来，纯 RL 研究更多集中在机器人和游戏 AI，普通转行者没必要把它当主线。
