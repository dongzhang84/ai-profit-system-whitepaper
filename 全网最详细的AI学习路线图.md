...
先说方法论。学 AI 最容易犯的错是漫天无边地乱学：看到什么新概念就想补一下，看到什么新论文就想读一遍。一年过去还没落地一个能跑的东西。正确的顺序是先**谋定而后动**：选定方向，选定目标岗位，再决定要学什么。比如"我要做 LLM 方向、面向工程岗的工作"。有了这个定位，知识的边界就清晰了，边界之外的先放着。

学习方式上我只信一个原则：**项目驱动学习（project-based learning）**，也可以叫主动学习。你通过动手做一个具体能跑的东西来掌握需要用到的知识。大部头的书和长视频教程是查漏补缺用的工具，不应该当成学习的主路。

具体节奏上，我借用"三三制"的思路：**每做三个 project，升一层能力**。下面以"LLM 方向 + 工程岗"为例，给你排一份从入门到能拿 offer 的阶梯。
