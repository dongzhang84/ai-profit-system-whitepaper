...
当然事情正在悄悄变化。尤其今年，公司 AI 化（本质是 agent 化）的口号越来越响。以前用规则加树模型做风控的场景，开始出现 LLM 辅助决策；以前靠关键词匹配做客服工单分类的，开始被 agent 接管。传统 ML 短期内不会消失，但它的地盘在 2026 年每个月都在被切走一小块。

## 1.2 计算机视觉（CV）

CV 是让机器看懂图片和视频。最基本的任务是分类，判定一张图是猫还是狗。更难的是检测和分割，要把图里每个物体框出来、或者按像素切成块。再难一步是生成，让机器从无到有画出一张图。十年前这些活清一色靠 CNN 这种卷积神经网络，近五年主力换成了 Vision Transformer，图像生成这一支则被 diffusion models 接管。

CV 岗位的需求在 2026 年还在，但增长明显放缓。Stable Diffusion 和 Midjourney 这类工具把相当一部分生成任务变成了 prompt 工程师的活。如果你有明确的垂直行业方向，比如医疗影像、自动驾驶、工业质检、机器人视觉，CV 依然是硬需求。但如果只是泛泛学个 ImageNet 分类，今天就业竞争力已经不强。
