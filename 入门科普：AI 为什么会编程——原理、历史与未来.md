...
回到 AI 这边，它今天能直接吃掉的环节其实不止写代码这一个。把每个阶段里 AI 真正能切的部分列一下。

需求阶段，AI 可以把零碎想法整理成 PRD 初稿，可以帮你做需求评审里的漏洞找茬（这个需求和已有功能冲突吗、缺哪个边界条件）。但能不能拍板、能不能跟业务方对齐，AI 做不了，那是决策权和责任的事。

设计阶段，UI 这一段被 v0、Figma AI 这一类工具吃得很透，从描述到能跑的组件几分钟。技术方案这一段，AI 可以列出来三套候选方案、各自的优缺点，但选哪一套要人来定，因为选型背后是一堆 AI 不知道的组织约束（团队熟悉哪个栈、合规要求、成本预算）。

开发阶段是 AI 真正的主战场，前面两章已经讲透。

测试阶段，AI 可以自动生成测试用例、跑回归、扫边界条件、做 fuzzing，效果非常好。真实用户验收（UAT）AI 替不了，那需要真人在真场景里点一遍。

上线阶段，AI 已经能做监控告警、异常检测、自动回滚这些事，但灰度策略要不要继续推、要不要全量、出事时要不要熔断，这些 judgment call 仍然是人在拍。

验证回收阶段，AI 可以拉数据、生成报表、给出几种可能的解释。但为什么某个功能转化率没达到预期这种归因，AI 只能给候选答案，最终判断还是人在做。

把上面这些加起来：AI 今天能直接替的有两块。一是写代码这一环里的大头（前两章讲过）；二是每个阶段里机械、有标准答案的子任务，比如生成测试、写文档、拉数据、做监控。每个阶段里需要决策权、需要责任、需要跨人共识的那一层，AI 替不了。
