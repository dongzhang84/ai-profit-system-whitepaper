...
### 3.1.2 设计阶段（UI/UX + 评审 + 技术方案 + 技术评审）

UI/UX 这一块被 v0、Figma AI 这一类工具吃得最透，从一句描述到能跑的 React 组件几分钟出，连样式系统都给你生成对。设计评审里"风格是否对齐""组件是否复用了已有库"这种形式化检查 AI 可以辅助，但"这个交互符合品牌调性吗""用户在这个流程的下一步会做什么"这种判断要资深设计师拍。技术方案这一段，AI 可以列出三套候选架构、对比各自的吞吐 / 延迟 / 成本，但选哪一套要人来定，因为选型背后是一堆 AI 不知道的组织约束（团队熟悉哪个栈、合规要求、SLA 承诺、人员稳定性）。技术评审会上"为什么不用 X、不用 Y"这种基于团队语境的拉锯辩论，AI 跟不上节奏。

### 3.1.3 开发阶段（任务拆分 + 前后端开发 + 联调 + 代码评审）

这是 AI 真正的主战场。任务拆分这一步 Claude Code 已经能从 PRD 直接生成 issue list 和依赖图。前后端开发是 Cursor + Claude / Codex 的核心使用场景，资深工程师里 2 到 5 倍的效率提升是普遍体感。联调（前后端打通接口）AI 能自动起 mock server、跑契约测试、扫接口签名不一致。代码评审 AI 可以做静态分析、规约检查、潜在 bug 标记，但"这个改动会不会让模块边界变模糊""这套抽象未来三年好不好维护"这种架构层面的 review 还是要资深 reviewer。整体看下来，开发阶段 70% 到 80% 的活 AI 能直接做，剩下的 20% 到 30% 是架构判断和疑难调试。
